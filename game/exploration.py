...
class ExplorationResult:
    def __init__(self, lines=None, combat_triggered=False, aggressors=None):
        self.lines = lines or []; self.combat_triggered = combat_triggered; self.aggressors = aggressors or []
...
class ExplorationController:
    def __init__(self, parser: CommandParser, registry: CommandRegistry, player: Player, world: WorldMap,
        ...
        self.parser = parser; self.registry = registry; self.player = player; self.world = world
        self.puzzle_flags = puzzle_flags or {}; self.item_catalog = item_catalog or {}
        self._handlers = {
            "go": self._go, "look": self._look, "open": self._object_command, "close": self._object_command,
            "unlock": self._object_command, "smash": self._object_command, "push": self._object_command,
            "pull": self._object_command, "move": self._object_command, "move_object": self._object_command,
            "put": self._put, "take": self._take, "drop": self._drop, "equip": self._equip, "unequip": self._unequip,
            "inventory": self._inventory, "help": self._help, "read": self._read, "compass": self._compass
        }
    ...
    def player_input(self, raw: str) -> ExplorationResult:
        ...
        if not parsed.valid:
            ...
            for line in apply_turn_passives(self.player, self.world, self.world.current_room_id): result.add(line)
            aggressors = self.world.enemies_visible_from_current()
            if aggressors: result.combat_triggered = True; result.aggressors = aggressors
            else: result.lines.extend(self.world.step_enemy_outside_combat(self.world.current_room_id))
            return result
        intent = (parsed.intent or "").lower()
        handler = self._handlers.get(intent)
        if handler: handler(parsed, result)
        else:
    ...
    def _go(self, parsed: ParsedCommand, result: ExplorationResult):
        ...
        if not direction: result.add("Go where?"); return
            ...
            if room and room.light_level > 7:
                ...
                if shield and "mirror_shield" in shield.item_flags:
                    ...
                    for obj in room.objects.values():
                        if obj.hidden: obj.hidden = False; revealed +=1
                    if revealed: result.add(f"Mirror Shield gleams and reveals {revealed} hidden object(s).")

    def _look(self, parsed: ParsedCommand, result: ExplorationResult):
        room = self.world.current_room()
        if not room: result.add("You are nowhere."); return
        target = (parsed.target_name or "").strip().lower()
        if not target: result.add(room.get_description(verbose=True, turn_number=self.world.turn_counter)); return
        if target in room.exits:
            adj = self.world.get_room(room.exits[target])
            result.add(adj.peek_description(room) if adj else "You see nothing that way."); return
        obj = room.find_object(target)
        if obj: result.add(obj.examine()); return
        result.add("You don't see that.")

    def _object_command(self, parsed: ParsedCommand, result: ExplorationResult):
        room = self.world.current_room()
        if not room: result.add("No room loaded."); return
        if not parsed.target_name: result.add("Target what?"); return
        obj = room.find_object(parsed.target_name)
        if not obj: result.add("No such object here."); return
        ok, reason = obj.can_interact_with(parsed.intent or "", self.player)
        if not ok: result.add(reason); return
        ...
        for key, value in obj.set_flags_on_interact.get(parsed.intent or "", {}).items():
            self.puzzle_flags[str(key)] = bool(value); self.world.global_flags[str(key)] = bool(value)
        for new_obj in revealed: room.add_object(new_obj); result.add(f"Revealed: {new_obj.name}.")

    def _take(self, parsed: ParsedCommand, result: ExplorationResult):
        room = self.world.current_room()
        if not room: result.add("No room loaded."); return
        target = (parsed.target_name or "").strip().lower()
        if not target: result.add("Take what?"); return
        # item on ground
        for item_id in room.items_on_ground:
            if target in item_id.lower():
                ...
                if item:
                    ...
                    return
                else: result.add(f"Unknown item: {item_id}"); return
        # moveable object
        obj = room.find_object(target)
        if obj and obj.is_moveable:
            item = EquippableItem(id=obj.id, name=obj.name, slot="misc", description=obj.description,
            ...
            if "compass" in item.item_flags:
                if not self.puzzle_flags.get("compass_unlock_shown", False):
                    self.puzzle_flags["compass_unlock_shown"] = True; self.world.global_flags["compass_unlock_shown"] = True
            ...
            for key in (obj.id, f"{obj.id}_desc"):
                if key in room.description_snippets: room.description_snippets[key] = ""
            return
        if obj and not obj.is_moveable: result.add(f"You cannot take the {obj.name}; it's fixed in place."); return
    ...
    def _put(self, parsed: ParsedCommand, result: ExplorationResult):
        ...
        if not room: result.add("No room loaded."); return
        target = (parsed.target_name or "").strip().lower()
        if not target: result.add("Put what where?"); return
        parts = re.split(r'\s+(?:in|into)\s+', target, maxsplit=1)
        if len(parts) != 2: result.add("Usage: put <item> in <container>"); return
        item_name, container_name = parts[0].strip(), parts[1].strip()
        container = room.find_object(container_name)
        if not container or not container.is_container: result.add(f"There is no container named '{container_name}' here."); return
        item = self.player.find_in_inventory(item_name)
        if not item: result.add(f"You don't have '{item_name}'."); return
        if container.id == "cabinet":
            ...
            if item.id not in container.items_inside: container.items_inside.append(item.id)
    ...
    def _unequip(self, parsed: ParsedCommand, result: ExplorationResult):
        ...
        if not slot: result.add("Unequip which slot?"); return
    ...
    def _help(self, parsed: ParsedCommand, result: ExplorationResult):
        ...
        if not cmds: result.add("No exploration commands available.")
        else: result.add("Available commands: " + ", ".join(sorted(c.name for c in cmds)))

    def _read(self, parsed: ParsedCommand, result: ExplorationResult):
        room = self.world.current_room()
        if not room: result.add("No room loaded."); return
        target = (parsed.target_name or "").strip().lower()
        if not target: result.add("Read what?"); return
        obj = room.find_object(target)
        if obj:
            text = obj.on_interact.get("read") or obj.on_interact.get("look")
            if text: result.add(text); return
        for item_id in room.items_on_ground:
            if target in item_id.lower():
                item = self.item_catalog.get(item_id)
                if item: text = getattr(item, "readable_text", None) or item.description
                else: text = None
                if text: result.add(text); return
                else: result.add(f"Unknown item: {item_id}"); return
    ...
    def _compass(self, parsed: ParsedCommand, result: ExplorationResult):
        if not any("compass" in it.item_flags for it in self.player.inventory+list(self.player.equipped.values())):
            result.add("You don't have a compass."); return
        room = self.world.current_room()
        if not room: result.add("You are nowhere."); return
        exits = room.exits
        if not exits: result.add("Your compass spins aimlessly – there are no visible exits."); return
    ...
    def _resolve_data_rule(self, parsed: ParsedCommand, result: ExplorationResult) -> bool:
        ...
        if self.player.level < int(req.get("min_level", 0)):
            result.add(rule.get("blocked_text", f"You need to be level {req['min_level']} to do that.")); return True
        class_req = req.get("class")
        class_id = getattr(getattr(self.player, "char_class", None), "id", None)
        if class_req and class_id != class_req:
            result.add(rule.get("blocked_text", "You are not trained for that.")); return True
        text = rule.get("text")
        if text: result.add(str(text))
        for obj_id in rule.get("reveal_objects", []):
            if obj_id in room.objects: room.objects[obj_id].hidden = False; result.add(f"You reveal {room.objects[obj_id].name}.")
        for item_id in rule.get("spawn_items", []):
            if item_id not in room.items_on_ground: room.items_on_ground.append(item_id)
        for direction, target_room in rule.get("set_exits", {}).items(): room.exits[str(direction).lower()] = str(target_room)
        for direction in rule.get("remove_exits", []): room.exits.pop(str(direction).lower(), None)
        for key, value in rule.get("set_flags", {}).items():
            self.puzzle_flags[str(key)] = value; self.world.global_flags[str(key)] = bool(value)
        return True
    ...
    def _resolve_garnish_rule(self, raw: str, result: ExplorationResult) -> bool:
        ...
        if target:
            obj = room.find_object(target)
            if obj: result.add(f"You try to {command} the {obj.name}, but nothing interesting happens."); return True
        elif command in {"pray","sing","dance","kneel","wave"}:
            result.add(f"You {command} for a moment. The world keeps its own counsel."); return True
        return False