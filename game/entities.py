...
class Entity:
    ...
    def __post_init__(self):
        if self.current_hp is None: self.current_hp = self.max_hp
    ...
    def add_block(self, amount: int) -> None: self.block = max(0, self.block + amount)
    def clear_block(self) -> None: self.block = 0
    def apply_effect(self, effect) -> str: return self.effects.apply(effect, self)
    def tick_effects(self, trigger) -> List[str]: return self.effects.tick_all(trigger, self)
...
class Player(Entity):
    ...
    def reset_ap(self) -> None: self.current_ap = self.total_ap
    def spend_ap(self, amount: int) -> bool:
        if amount > self.current_ap: return False
    ...
    def unlock_command(self, command_name: str) -> None: self.unlocked_commands.add(command_name)
    def add_score(self, amount: int) -> None: self.score += amount
    def base_attack_value(self) -> int:
        return self.attack + self.effects.stat_bonus("attack")
    def weapon_attack_bonus(self, weapon_name: Optional[str] = None) -> int:
        if not weapon_name: return 0
    ...
    def equip(self, item_name: str) -> str:
        ...
        if not item: return f"You don't have '{item_name}'."
        ok, reason = self.can_equip(item)
        if not ok: return reason
        previous = self.equipped.get(item.slot)
        if previous: self._remove_dynamic_item_effects(previous)
    ...
    def unequip(self, slot: str) -> str:
        ...
        if not item: return f"Nothing equipped in {slot}."
    ...
    def attack_value(self) -> int:
        base = self.attack + sum(i.stat_modifiers.get("attack", 0) for i in self.equipped.values())
        return base + self.effects.stat_bonus("attack")

    def defense_value(self) -> int:
        base = self.defense + sum(i.stat_modifiers.get("defense", 0) for i in self.equipped.values())
        return base + self.combat_defense_bonus + self.effects.stat_bonus("defense")
    ...
    def ap_cost_reduction_for_text(self, raw_text: str) -> int:
        ...
        total = 0
        for item in self.equipped.values():
            for ch, amount in item.letter_cost_reductions.items():
    ...
    def drop(self, item_name: str) -> Tuple[Optional[EquippableItem], str]:
        item = self.find_in_inventory(item_name)
        if item:
    ...
    def find_in_inventory(self, name: str) -> Optional[EquippableItem]:
        needle = name.lower()
        for item in self.inventory:
            if needle in item.name.lower(): return item
        return None
    ...
    def level_up(self):
        ...
        if self.char_class:
            ...
            for cmd in unlocked: self.unlock_command(cmd)
        lines = [f"Level up! You are now level {self.level}."]
        if unlocked: lines.append("Unlocked: " + ", ".join(unlocked))
    ...
    def inventory_summary(self) -> str:
        ...
        if self.inventory:
            for item in self.inventory: lines.append(f"  - {item.name}")
        else: lines.append("  (empty)")
        return "\n".join(lines)
...
class Enemy(Entity):
    ...
    trap_room_id: Optional[str] = None
    ...
    def _check_condition(self, condition: Optional[str], player: Player) -> bool:
        cond = (condition or "always").strip().lower()
        if cond in ("", "always"): return True
        if cond == "hp_below_half": return self.hp_fraction < 0.5
    ...
    def intent_display(self) -> str:
        if not self.active_intents: return "(no intents)"
        return ", ".join(i.id for i in self.active_intents)

    def choose_world_move(self, world, player_zone: Optional[str] = None) -> Optional[str]:
        if self.current_zone is None: return None
        profile = self.ai_profile.lower()
        neighbors = [n for n in world.neighbors_of(self.current_zone) if n not in self.forbidden_zones]
        ...
        if profile == "trap" and not self.trap_placed:
            ...
            if target:
                ...
                if len(path) > 1: return path[1]
                    ...
                    self.trap_placed, self.trap_room_id = True, target
                    return None
            return random.choice(neighbors) if neighbors else None
        if profile == "patrol" and self.patrol_points:
            target = self.patrol_points[self.patrol_index % len(self.patrol_points)]
            if self.current_zone == target: self.patrol_index += 1
            path = world.shortest_path(self.current_zone, target, blocked=self.forbidden_zones)
            return path[1] if len(path) > 1 else None
        if profile == "wander": return random.choice(neighbors)
        if profile == "hunter" and player_zone:
            path = world.shortest_path(self.current_zone, player_zone, blocked=self.forbidden_zones)
            if len(path) - 1 < 10: return path[1] if len(path) > 1 else random.choice(neighbors)
            return None
