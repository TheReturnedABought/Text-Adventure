...
class CommandDefinition:
    ...
    aliases: List[str]
    description: str
    ap_cost_override: Optional[int] = None
    mp_cost_override: Optional[int] = None
    costs_mp: bool = False
    base_dice: Optional[str] = None
    unlock_class: Optional[str] = None
    unlock_level: int = 0
    ...
    tags: List[str] = field(default_factory=list)
    world_use_hint: Optional[str] = None
    base_ap_cost: int = 0
    base_mp_cost: int = 0
...
class CommandRegistry:
    ...
    def load_from_dict(self, data: dict) -> None:
        for raw in data.get("commands", []):
            ...
            valid = [c for c in contexts if c] or [CommandContext.COMBAT, CommandContext.WORLD]
        ...
        for raw in data.get("modifiers", []):
            contexts = [self._coerce_context(c) for c in raw.get("valid_contexts", [])]
            valid = [c for c in contexts if c] or [CommandContext.ANY]
    ...
    def get_command(self, verb: str) -> Optional[CommandDefinition]:
...
class UnlockTable:
    ...
    def add_unlock(self, class_name: str, level: int, command: str) -> None:
        self._level_unlocks.setdefault(class_name.lower(), {}).setdefault(level, []).append(command)

    def add_choice_unlock(self, class_name: str, level: int, choices: List[str]) -> None:
        self._choice_unlocks.setdefault(class_name.lower(), {}).setdefault(level, []).append(list(choices))
